...
**Goal:** Build a CPU-only Dockerized Gradio WebUI that lets a phone browser upload one image, choose an installed model, upscale it, preview the result, and download the output.
...
- Create `app/model_registry.py`: defines model metadata, filters installed models, and resolves a selected model id.
- Create `app/inference.py`: validates uploaded images, persists input files, resolves models, calls a backend, saves outputs, and returns status metadata.
- Create `app/app.py`: builds the mobile-friendly Gradio interface and maps inference results/errors to UI components.
- Create `app/Dockerfile`: builds a CPU-only runtime image.
- Create `app/requirements.txt`: pins runtime and test dependencies.
- Create `compose.yml`: runs the app with loopback-only host port binding and persistent volumes.
...
- Default public safety posture is loopback-only Docker port binding: `127.0.0.1:7860:7860`.
...
Expected: service starts and binds `127.0.0.1:7860`.
