...
如果请求在入队前被拒绝，例如没有图片、输出尺寸非法、模型不可用或模型文件缺失，
API 会返回结构化 `400` 错误并写入 `ui_rejected` JSONL 日志。此类错误有
`request_id` 便于查日志，但不会创建 SQLite task row。
